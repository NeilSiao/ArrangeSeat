...
@section('content')
    ...
    <h3>Create studentGroup Data</h3>
    <div class="row">
        <div class="col-auto my-2">
            <a href="{{route('studentGroup.create')}}" class="btn btn-primary m-1">Add studentGroup</a>
            <a href="{{route('studentGroup.excel')}}" class="btn btn-secondary m-1">Download Excel</a>
        ...
        <tbody>
            @if ($studentGroups->isNotEmpty())
                @foreach ($studentGroups as $studentGroup)
                    <tr>
                        <th class="align-middle" style="width: 10%" scope="row">{{ $studentGroup->id }}</th>
                        <td class="align-middle" style="width: 20%">{{ $studentGroup->no }}</td>
                        <td class="align-middle" style="width: 15%"><img width="64px" height="64px"
                                src="{{ $studentGroup->photo }}" alt=""></td>
                        <td class="align-middle" style="width: 20%">{{ $studentGroup->name }}</td>
                        <td class="align-middle" style="width: 20%">{{ $studentGroup->gender }}</td>
                        <td class="align-middle" style="width: 20%">
                            <div class="d-flex justify-content-around">
                                <button class="btn-sm btn-warning" data-id="{{ $studentGroup->id }}" data-toggle="modal"
                                    onclick="edit(this)">Edit</button>
                                <form action="{{ route('studentGroup.destroy', $studentGroup->id) }} }}" method="post" id="deleteRow">
                                    @csrf
    ...
    <script >
        ...
        function edit(evt) {
            ...
            $('#editModal').show();
            var editFormAction = "{{ route('studentGroup.index') }}" + '/' + idText;
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Modify studentGroup Data</h5>
